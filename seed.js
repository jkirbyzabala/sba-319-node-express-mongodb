...
const sampleUsers = [
  ...
  },
  {
    ...
    password: "daymon",
    ratings: "8/10",
    reason: "kind person, questionable relations within family"
  },
];
...
mongoose.connect(process.env.MONGO_URI, { 
  ...
  .then(async () => {
    ...
    console.log('Sample users inserted:', userResult);

    const reviewResult = await Review.insertMany(sampleReviews);
    console.log('Sample reviews inserted:', reviewResult);

    const actorResult = await Actor.insertMany(sampleActors);
    console.log('Sample actors inserted:', actorResult);
  })
